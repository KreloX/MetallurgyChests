...
public class TileEntityPlatinumChest extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.platinum_chest";
	}
	...
	{
		return Reference.MODID + ":platinum_chest";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerPlatinumChest)
                {
                    if (((ContainerPlatinumChest)entityplayer.openContainer).getChestInventory() == this)
                    {
