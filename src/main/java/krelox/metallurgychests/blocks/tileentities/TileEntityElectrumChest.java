...
public class TileEntityElectrumChest extends TileEntityLockableLoot implements ITickable
{
	private NonNullList<ItemStack> chestContents = NonNullList.<ItemStack>withSize(81, ItemStack.EMPTY);
	...
	{
		return this.hasCustomName() ? this.customName : "container.electrum_chest";
	}
	...
	{
		return Reference.MODID + ":electrum_chest";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerElectrumChest)
                {
                    if (((ContainerElectrumChest)entityplayer.openContainer).getChestInventory() == this)
                    {
