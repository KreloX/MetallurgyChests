...
public class TileEntityRubraciumChest extends TileEntityLockableLoot implements ITickable
{
	private NonNullList<ItemStack> chestContents = NonNullList.<ItemStack>withSize(90, ItemStack.EMPTY);
	...
	{
		return this.hasCustomName() ? this.customName : "container.rubracium_chest";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerRubraciumChest)
                {
                    if (((ContainerRubraciumChest)entityplayer.openContainer).getChestInventory() == this)
                    {
