...
public class ContainerRubraciumChest extends Container
{
	...
	public ContainerRubraciumChest(InventoryPlayer playerInv, TileEntityRubraciumChest chestInventory, EntityPlayer player) 
	{
		...
		this.numRows = chestInventory.func_70302_i_() / 10;
		...
		for(int i = 0; i < this.numRows; ++i) 
		{
			for(int j = 0; j < 10; ++j) 
			{
				this.func_75146_a(new Slot(chestInventory, j + i*10, 8 + j*18, 20 + i*18));
			}
		}
		...
		for(int y = 0; y < 3; y++) 
		{
			for(int x = 0; x < 9; x++) 
			{
				this.func_75146_a(new Slot(playerInv, x + y*9 + 9, 17 + x*18, 196 + y*18));
			}
		}
		
		for(int x = 0; x < 9; x++) 
		{
			this.func_75146_a(new Slot(playerInv, x, 17 + x*18, 254));
		}
	}
