...
public class ContainerRubraciumChest extends Container
{
	...
	public ContainerRubraciumChest(InventoryPlayer playerInv, TileEntityRubraciumChest chestInventory, EntityPlayer player) 
	{
		...
		for(int i = 0; i < this.numRows; ++i) 
		{
			for(int j = 0; j < 10; ++j) 
			{
				this.addSlotToContainer(new Slot(chestInventory, j + i*10, 8 + j*18, 20 + i*18));
			}
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
	{
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            if (index < this.numRows * 10)
            {
                if (!this.mergeItemStack(itemstack1, this.numRows * 10, this.inventorySlots.size(), true))
                {
                    return ItemStack.EMPTY;
                }
            }
            else if (!this.mergeItemStack(itemstack1, 0, this.numRows * 10, false))
            {
                return ItemStack.EMPTY;
            }
